...
## 2026-06-10 — Main screen layout (prototype verdict)
**What:** Main screen switched from kanban-primary to terminal-primary (~90/10). Tabs stay on top; the terminal grid fills the screen (new `TerminalArea` with header actions: New terminal, notifications, settings); the kanban is reduced to a slim 56px bottom dock (new `KanbanDock`: per-column card counts + "Doing" card chips) that opens a 60vh drawer containing the full `Board` (drag-and-drop, card detail and quick-add intact). Decision came from a 3-variant throwaway UI prototype on the main route (A: 50/50 split, B: 70/30 terminal + vertical kanban rail, C: 90/10 terminal + dock/drawer) — variant C won; prototype deleted.
**Why:** The terminal is the primary working surface; the board is glanceable status that's only occasionally manipulated, so it moves to the periphery (dock) with full functionality one click away (drawer).
**Where:** `src/App.tsx` (layout), `src/components/TerminalArea.tsx` (new), `src/components/KanbanDock.tsx` (new), `src/components/Board.tsx` (COLUMN_ORDER exported for reuse).
