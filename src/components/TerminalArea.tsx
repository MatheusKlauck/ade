...
// per-window session id (learned from the Claude hook markers, see App.tsx)
...
export default function TerminalArea({
	...
	const doReorder = useCallback(
		(draggedWin: string, targetWin: string, edge: "before" | "after") => {
			...
			const base = normalizeLayout(
				st.layoutByWorkspace[workspaceId] ?? [],
				wins,
			);
			setLayout(
				workspaceId,
				reorderLayout(base, draggedWin, targetWin, edge),
				true,
			);
		},
	...
	// reported (sessionIdByWindow). A window with no sid is NOT running Claude
	// (or hasn't fired its first hook yet), so it must not inherit another
	// window's session title — return a neutral, window-unique placeholder
	// instead. Previously this fell back to the workspace's newest-active
	// session, which made non-Claude terminals and pre-hook Claude windows all
	// share the same borrowed title.
	const sessionFor = (pane: OpenTerminal): SessionInfo | undefined => {
		const sid = sessionIdByWindow[pane.windowId];
		if (!sid) {
			return { title: `Terminal @ ${pane.windowId}`, branch: null };
		}
	...
	// Windows that share the same Claude session (e.g. two `claude --resume <id>`
	// panes) would otherwise render the same title. Map each known sid to the
	// windows running it so `titleFor` can append a `(window @ <id>)`
	// disambiguator when a sid is shared.
	const windowsBySid = useMemo(() => {
		const m = new Map<string, string[]>();
		for (const p of panes) {
			const sid = sessionIdByWindow[p.windowId];
			if (!sid) continue;
			const arr = m.get(sid) ?? [];
			arr.push(p.windowId);
			m.set(sid, arr);
		...
		return m;
	}, [panes, sessionIdByWindow]);

	// Title: custom name if set, else the linked card/issue (e.g. "#123 | Fix
	// login"), else the active Claude session's title (disambiguated by window
	// when two panes share the same session), else a neutral "Terminal @ <window>".
	const titleFor = (pane: OpenTerminal): string => {
		...
		if (sess && sess.title && sess.title !== "(untitled session)") {
			const sid = sessionIdByWindow[pane.windowId];
			// No sid → sess.title is already the neutral `Terminal @ <windowId>`.
			if (!sid) return sess.title;
			// Two windows running the same Claude session (e.g. `--resume <id>`)
			// would otherwise render identical titles — disambiguate by window.
			const sharers = windowsBySid.get(sid);
			if (sharers && sharers.length > 1) {
				return `${sess.title} (window @ ${pane.windowId})`;
			}
			return sess.title;
		}
		return "Terminal";
	...
	const branchFor = (pane: OpenTerminal): string | null => {
		...
		if (card?.github_issue_number != null)
			return `issue-${card.github_issue_number}`;
		return null;
	};
							...
							return (
								...
								>
									<TerminalPane
										...
										onToggleMinimize={
											paneHandlers.get(pane.paneId)?.onToggleMinimize
										}
										onToggleMaximize={
											paneHandlers.get(pane.paneId)?.onToggleMaximize
										}
